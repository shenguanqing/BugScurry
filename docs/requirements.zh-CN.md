...
| F-BUG-05 | 点击「全部清除」：清空所有虫子（并停天气） | P0 |
...
| 编号 | 需求 | 优先级 |
|------|------|--------|
| F-PER-01 | 出生随机四种性格：胆小 / 贪吃 / 慵懒 / 好奇 | P0 |
| F-FEED-01 | 托盘「投喂」：饼干 / 糖 / 水果；最多同时三份 | P0 |
| F-FEED-02 | 虫种最爱食物；吃最爱时显示爱心 | P0 |
| F-FEED-03 | 性格 + 虫种吃相（短啄、粘食、蚂蚁搬运等） | P1 |
| F-FEED-04 | 啃食碎屑 / 糖闪光 / 果汁印；餐后暖光 | P1 |
| F-WX-01 | 本机时钟时段偏好影响随机混养 | P1 |
| F-WX-02 | 托盘天气：停止 / 小雨 / 中雨 / 大雨 / 暴雨 / 雷阵雨 / 雪 / 雾 / 沙尘（种类随机只在设置「随机天气」里） | P0 |
| F-WX-03 | 托盘勾选当前天气；每场重掷风向；雷电与环境声 | P1 |
| F-WX-04 | 设置「随机天气」自动节奏；手动选择会重置时钟 | P1 |
| F-WX-05 | 环境声与「捏死音效」共用总开关（雪/雾近无声，沙尘偏风噪） | P1 |

### 3.5.1 喷雾与随机事件（0.3.x）

| 编号 | 需求 | 优先级 |
|------|------|--------|
| F-PRANK-01 | 托盘「喷雾杀虫剂」：当前各屏活虫一次击杀（大肥虫算一杀），短雾特效 + 喷雾音效（共用音效开关）；冷却 45s，文案显示剩余秒 | P0 |
| F-PRANK-02 | 设置「随机事件」：约每 90–240s 等概率抽一种（虫潮 / 大肥虫入侵 / 狂暴爬行 / 体型乱变 / 夜袭）；任何事件都**不改写** store 的 `count` | P0 |
| F-PRANK-03 | 触发前全屏标题卡播完并淡出后再执行；隐藏虫子或虫潮进行中则顺延 | P0 |
| F-PRANK-04 | 喷雾广播到所有覆盖层；自动事件由主覆盖层调度并广播 kind，各屏本地执行 | P0 |
| F-PRANK-05 | 隐藏调试面板（连点页脚 5 次）：逐档天气 / 停止、逐个触发事件、喷雾 | P2 |
...
| F-SET-08 | 一键「全部清除」（虫子、食物、痕迹，并停天气） | P0 |
| F-SET-09 | 一键「重新生成」 | P0 |
| F-SET-10 | 开机启动 | P1 |
| F-SET-11 | 始终运行（架构保证的托盘常驻：关闭设置不退出；仅托盘「退出」结束进程） | P1 — 架构能力，无独立开关 |
| F-SET-12 | 多显示器：当前屏幕 / 所有屏幕 | P1 |
| F-SET-13 | 设置修改即时生效并持久化 | P0 |
| F-SET-14 | 随机事件开关（自动抽事件池）；说明为列表弹层 | P0 |
